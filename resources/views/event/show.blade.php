<x-app-layout>
    <div class="container mx-auto py-8 text-black">
        ...
        @auth
            <form action="{{ route('events.toggleFavorite', $event->id) }}" method="POST" class="mt-6">
                @csrf
                <button type="submit"
                    class="px-4 py-2 font-semibold rounded border-b-2 border-transparent bg-white text-black hover:border-red-500 transition">
                    {{ auth()->user()->favoriteEvents->contains($event->id) ? 'Unsave' : 'Save' }}
                </button>
            </form>
        @endauth
